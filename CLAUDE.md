...
- **Lógica Rescate 1 simplificada:** `_procesar_lead` ya no usa cadencias por stage. Única condición: 30+ min de silencio del usuario. Aplica a cualquier stage excepto `C90:WON`, `C90:1`, `C90:2`, `C90:3`. El filtro SQL usa `$1` como único parámetro cuando `SEGUIMIENTOS_TEST_PHONE` está activo (antes usaba `$2` causando error de BD).
