...
| Manual Dashboard + Bitrix | [`docs/manual_dashboard_bitrix_integracion.md`](docs/manual_dashboard_bitrix_integracion.md) | Manual paso a paso para usuarios y administradores: acceso al dashboard, detalle de conversación, configuración del embed en Bitrix24, OAuth placement y mantenimiento. |
| Estructura de tablas | [`docs/estructura_tablas.md`](docs/estructura_tablas.md) | Esquema completo de todas las tablas PostgreSQL: columnas, tipos, propósito, quién las usa y consultas de cruce útiles. |
...
- **Fix "deal duplicado en Open Lines" (2026-07-16):** el fix anterior solo cubría el disparador de `job_seguimientos`. Quedaba un segundo camino: cuando el bot espeja un mensaje a Bitrix (`send_user_message` o `send_bot_message` vía imconnector) sobre un deal cuya sesión de Open Lines ya está cerrada (deal en `C90:WON`/`C90:LOSE`), Bitrix trata el mensaje como "cliente nuevo" y abre una sesión — y un deal — duplicado. Esto ocurría cada vez que `job_seguimientos` mandaba un rescate sobre un deal recién cerrado (antes del fix del 07-15) y también puede ocurrir si el cliente le escribe orgánicamente a Vera días después de que su deal ya cerró. Se detectaron y corrigieron manualmente 7 deals duplicados (WON real + hasta 3 duplicados por caso) del 14 y 15 de julio. Fix: en `api/routes/webhooks.py`, antes de espejear cualquier mensaje (`receive_message` y `_process_message`) se consulta `leads.bitrix_stage`; si es `C90:WON` se omite el espejeo (el bot sigue respondiendo solo por WhatsApp); si es `C90:LOSE` se reactiva el deal a `C90:PREPAYMENT_INVOIC` (Recuperación) antes de espejear, para no abrir la sesión sobre un deal cerrado.
  - **Verificación en vivo (2026-07-16):** probado end-to-end con el teléfono de prueba `593991053639` sobre el deal `2428056` en producción. Caso `C90:WON`: se escribió al bot con el deal ya ganado → log `webhook_skip_mirror_deal_won` se disparó, no se llamó a `send_user_message`, cero deals nuevos creados para ese número. Caso `C90:LOSE`: se escribió al bot con el deal en Caído → log `webhook_deal_reactivado` se disparó, el deal pasó a `C90:PREPAYMENT_INVOIC` y el mensaje se espejeó sobre la misma `session_id` de Open Lines existente (no una nueva), sin deal duplicado. Ambos escenarios confirmados sin regresión.
...
- **Revert de la eliminación del dashboard (2026-07-13):** el commit `b8d3274` (2026-07-10) había eliminado el dashboard Angular, `api/routes/auth.py`, `jobs/kpi_export.py`, `jobs/email_report.py` y `scripts/seed_dashboard_users.py` para "reconstruir con un stack nuevo". El dashboard y el reporte diario seguían en uso activo, así que se revirtió por completo (`git revert b8d3274`, commit `416670b`) — todo el stack descrito en este documento (dashboard, dos jobs de KPI, auth JWT) está de nuevo activo en producción tal como estaba antes. Verificado tras el revert: build de `api`/`dashboard`, login de los 3 usuarios, `/admin/kpi-data`, envío manual de `job_kpi_export` (107 conversaciones, 0 errores), `job_bitrix_sync` (729/741 actualizados) y `connector_poll` (reenvío de mensajes de asesor en vivo), todos funcionando correctamente.
- **Fix `/auth/me` (2026-07-13):** el parámetro `authorization: str = ""` en `api/routes/auth.py` no usaba `Header(...)`, así que FastAPI lo interpretaba como query param en vez de leer el header `Authorization: Bearer <token>` real. El frontend nunca llama este endpoint (solo usa `/auth/login`), pero se corrigió a `authorization: str = Header("")` para que funcione con el header estándar.
